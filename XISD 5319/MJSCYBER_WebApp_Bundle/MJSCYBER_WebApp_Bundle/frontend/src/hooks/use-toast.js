...
// --- Small, focused reducer helpers (keep each case < 50 lines) ---
...
function useToast() {
  ...
  React.useEffect(() => {
    ...
    // `listeners` is a module-level stable reference and `setState` from
    // useState is guaranteed stable by React, so an empty dependency array
    // is correct here — the effect should only subscribe/unsubscribe once.
